...
func TestA(t *testing.T) {
	...
	for i, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			got := topKFrequent(test.inputNums, test.inputK)
			if !arrSameJudge(got, test.want) {
...
func TestB(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "adjust1",
			inArr:   []int{1, 2, 2, 5, 3, 2, 3},
			inStart: 0,
			inEnd:   7,
			inVal:   4,
			out:     []int{2, 3, 2, 5, 4, 2, 3},
		},
	}
	for i, test := range tests {
		if i < 0 {
		...
		t.Run(test.name, func(t *testing.T) {
			test.inArr[0] = test.inVal
			adjustHeap(test.inArr, test.inStart, test.inEnd)
			if !reflect.DeepEqual(test.inArr, test.out) {
				t.Errorf("error")
			}
		})
	}
